...
SFDA — TFA Scraper
==================
Scrapes all TFA medical device records from the SFDA API.
Run alongside sfda_GHTF.py and sfda_Lowrisk.py in separate terminals.

Output : sfda_TFA.csv
State  : sfda_TFA_progress.json   ← resumes if script is closed
Log    : sfda_TFA.log
"""
...
REQUEST_DELAY = 0.5   # seconds between every page (keeps server happy)
MAX_RETRIES   = 6
RETRY_DELAY   = 5
TIMEOUT       = 30
RESET_BACKOFF = 15    # extra wait after a connection reset
JUMP_AFTER    = 30    # consecutive 404s before skipping ahead
JUMP_SIZE     = 200   # pages to skip when a dead zone is detected
SAVE_EVERY    = 50    # pages between progress saves
...
def run():
    ...
    # Fetch page 1 for metadata + column discovery
    ...
    with CSV_PATH.open(file_mode, newline="", encoding="utf-8-sig") as fh:
        ...
        # Write page 1 data if starting fresh
